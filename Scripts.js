...
// Change carousel speed for mobile
function startCarouselAutoPlay() {
    stopCarouselAutoPlay(); 
    
    // Check if user is on mobile
    const isMobile = window.innerWidth < 768;
    const speed = isMobile ? 4000 : 2000; // 4 seconds on mobile, 2 on desktop
...
// Slow down the Boxer Prism rotation
function resetBoxerTimer() {
    clearInterval(boxerInterval);
    if (!document.hidden) {
        const isMobile = window.innerWidth < 768;
        boxerInterval = setInterval(rotateBoxer, isMobile ? 5000 : 3000);
    }
}
